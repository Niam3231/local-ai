<?php
session_start();                                                                                                                              // === CONFIG ===                                                      $models_file = __DIR__ . '/models.json';                               $OLLAMA_URL = 'http://localhost:11434/api/chat';

// === LOAD MODELS ===
$models_file = __DIR__ . '/models.json';

// Predefined metadata for all known models
$MODEL_INFO = [
    'gemma3:270m' => [
        'name' => 'Gemma 3 270m',
...
if (file_exists($models_file)) {
    ...
    $raw = trim($raw, "\xEF\xBB\xBF"); // remove BOM if present
...
// === AVAILABLE MODELS MAP FOR SERVER VALIDATION ===
$AVAILABLE_MODELS = [];
...
/* === HEADER === */
#chat-header{
  background:#222635;padding:.7em 2vw;font-weight:bold;border-bottom:1px solid #24272d;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:.4em;min-height:42px;width:100%;
}
#chat-header .brand{
  color:#68a3ff;font-size:.85em;margin-left:6px;max-width:60vw;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;display:inline-block;vertical-align:middle;
}
...
let conversation=[];
function renderConversation(){chatBox.innerHTML='';for(const m of conversation)addMsg(m.role,m.content);chatBox.scrollTop=chatBox.scrollHeight;}
function addMsg(role,text){const msgDiv=document.createElement('div');msgDiv.className='msg '+role;const bubble=document.createElement('div');bubble.className='bubble';bubble.textContent=text;msgDiv.appendChild(bubble);chatBox.appendChild(msgDiv);chatBox.scrollTop=chatBox.scrollHeight;return bubble;}
function startNewChat(){fetch('?new=1',{method:'POST'}).then(()=>{conversation=[];chatBox.innerHTML='';msgInput.value='';msgInput.disabled=false;msgInput.focus();});}
